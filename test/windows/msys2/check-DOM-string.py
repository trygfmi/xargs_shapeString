# python /Users/ojiro/Desktop/programming/running-terminal-commands-blog/xargs/xargs_shapeString/test/windows/msys2/check-DOM-string.py
...
quickstart_code = tree.xpath('(//code[@class="code-flex"])[4]')
for i, el in enumerate(quickstart_code, 1):
    print("#####quickstart_code#####")
    print(el.text_content())
    print()

procedure_code = tree.xpath('(//code[@class="code-flex"])[16]')
for i, el in enumerate(procedure_code, 1):
    print("#####procedure_code#####")
    print(el.text_content())
    print()
    
quickstart_output_code = tree.xpath('(//div[@class="entry-content"]/details/pre[@class="wp-block-code has-background"]/code)[4]')
...
procedure_output_code = tree.xpath('(//details//details//code)[4]')
